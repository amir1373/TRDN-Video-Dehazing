...
| Checkpoint selection | `checkpoint_selection_metric=psnr` by default, matching the RunPod driver's `best_psnr` path. Both `best_psnr` and `best_ssim` diagnostic checkpoints remain tracked on the held-out training validation split. | The two best checkpoints can differ. Early stopping is implemented but disabled by default. | State the configured selection metric in reporting and never select on test metrics. Enable early stopping only as an explicit training-policy choice with a recorded patience. |
| Validation fidelity | Defaults are up to 32 validation samples and 30 DDIM steps every 500 training steps, with fixed `validation_seed=1234`. Actual samples, steps, U-Net forward passes, and measured wall time are recorded per pass. | This is substantially more expensive than the former 4-batch/10-step proxy, but reduces checkpoint-ranking noise. A small held-out split can still limit ranking confidence. | Keep the protocol fixed across ablations. Adjust sample count or steps only as a declared configuration change informed by paid-run cost. |
